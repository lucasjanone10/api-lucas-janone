/**
 * ============================================================================
 * SISTEMA GERADOR DE ROTEIROS DE ALTO LUXO - VERSÃO 25 (ENTERPRISE EDITION)
 * ARQUITETURA: LUCAS JANONE MENTORIA PREMIUM
 * ============================================================================
 * Este sistema incorpora resiliência de nível militar, design editorial "Vogue",
 * recuperação inteligente de falhas da IA e motores de busca de imagem em cascata.
 */
...
// ============================================================================
// 1. UTILITÁRIOS E CLASSES DE SUPORTE (LOGS E FORMATAÇÃO)
// ============================================================================
...
const formatadores = {
    ...
    extrairDias: (body) => {
        ...
        return numDias > 30 ? 30 : (numDias < 1 ? 5 : numDias); // Máximo de 30 dias para evitar colapso
    }
};

// ============================================================================
// 2. MOTORES DE BUSCA DE IMAGENS (SISTEMA EM CASCATA TRIPLA)
// ============================================================================
...
class ImageEngine {
    ...
    static async getBestImage(primaryQuery, fallbackQuery, index) {
        // Tenta 1: Wikipedia Exata (Monumento)
        let img = await this.getWikiImage(primaryQuery);
        if (img) return img;

        // Tenta 2: Wikipedia Cidade
        img = await this.getWikiImage(fallbackQuery);
        if (img) return img;

        // Tenta 3: Banco de Dados Fotográfico Rápido
        return this.getFallbackImage(primaryQuery || fallbackQuery, index);
    }
}

// ============================================================================
// 3. INTELIGÊNCIA ARTIFICIAL E AUTO-CURA (WOLVERINE PROTOCOL)
// ============================================================================

class AIEngine {
    static async selectBestModel(genAI) {
        try {
    ...
    static fixBrokenJSON(brokenString) {
        ...
        try {
            // Remove crases markdown de código
            let cleanStr = brokenString.replace(/```json/g, '').replace(/```/g, '').trim();
            
            // Se a IA foi cortada no meio, forçamos o fechamento do arquivo JSON
            ...
            // Injeta o final padrão se estiver faltando as chaves principais
            if (!fixedStr.includes('"segredos"')) {
                fixedStr = fixedStr.substring(0, fixedStr.lastIndexOf('}')); // Volta um passo
...
// ============================================================================
// 4. CONSTRUTOR DE DESIGN EDITORIAL (HTML E CSS GIGANTE)
// ============================================================================
...
class DocumentBuilder {
    static build(dados, destinoFormatado, nomeCliente, primeiraCidade, imagesMap) {
        ...
        <head>
            ...
            <link href="[https://fonts.googleapis.com/css2?family=Montserrat:wght@300;400;500;600&family=Playfair+Display:ital,wght@0,400;0,600;0,700;1,400&display=swap](https://fonts.googleapis.com/css2?family=Montserrat:wght@300;400;500;600&family=Playfair+Display:ital,wght@0,400;0,600;0,700;1,400&display=swap)" rel="stylesheet">
            <style>
                :root { 
                    --cream: #FDFCFB; 
                    --charcoal: #1A1A1A; 
                ...
                /* ESTRUTURA DE PÁGINA (SANGRIAS E MARGENS) */
                .page { 
                ...
                .cover-content { 
                    position: relative; z-index: 20; background: rgba(253, 252, 251, 0.92); 
                ...
                /* DIAGRAMAÇÃO DE CONTEÚDO (GRID E FLEX) */
                ...
                /* TIPOGRAFIA AVANÇADA E MARCAS D'ÁGUA */
...
// ============================================================================
// 5. ORQUESTRAÇÃO PRINCIPAL (ROTA DA API)
// ============================================================================
...
app.post('/api/gerar-roteiro', async (req, res) => {
    ...
    try {
        ...
        // Busca imagens em paralelo para velocidade máxima
        ...
        // LANÇAMENTO DO PUPPETEER (NÍVEL ENTERPRISE)
        ...
        try {
            Logger.info("Renderizando PDF. Aguardando 60 segundos máximos para mídia pesada...");
...
app.listen(PORT, () => {
    console.log(`✅ Robô V25 (Enterprise Edition) operando na porta ${PORT}`);
});