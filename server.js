...
app.post('/api/gerar-roteiro', async (req, res) => {
    try {
        ...
        // PROMPT DE IMAGENS MAIS LEVE PARA ECONOMIZAR MEMÓRIA NA NUVEM
        let imageCounter = 1;
        roteiroHTML = roteiroHTML.replace(/<h2(.*?)>(.*?)<\/h2>/g, (match) => {
            ...
            // Resolução reduzida para 600x300 (mais leve)
            const imgUrl = `https://image.pollinations.ai/prompt/${query}?width=600&height=300&nologo=true`;
            imageCounter++;
        ...
        // Capa levemente reduzida para 900x1200
        const coverImageUrl = `https://image.pollinations.ai/prompt/${coverQuery}?width=900&height=1200&nologo=true`;
        ...
        const browser = await puppeteer.launch({ 
            ...
            args: [
                ...
                '--disable-dev-shm-usage', // <-- MAGIA ANTI-CRASH DE MEMÓRIA
                '--disable-gpu',           // <-- ALIVIA A PLACA DE VÍDEO
