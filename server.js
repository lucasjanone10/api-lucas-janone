...
app.post('/api/gerar-roteiro', async (req, res) => {
    try {
        ...
        - PROIBIDO USAR CAPS LOCK em títulos ou textos. Use maiúsculas apenas no início das frases.
        ...
        // A MÁGICA ANTIBLOQUEIO: Usamos temas variados para a IA não travar as fotos!
        let imageCounter = 0;
        ...
        // A SUPER-COLA que impede o título de se separar da foto
        ...
        for (let parte of partes) {
            if (parte.startsWith('<h2')) {
                ...
                const query = encodeURIComponent(`cinematic travel photography ${temaAtual} in ${destino} 4k`);
        ...
        <head>
            ...
            <style>
                ...
                /* REGRA DE OURO: A Caixa que segura o título e a imagem juntos */
                .day-header {
        ...
        // A SOLUÇÃO DA FONTE: O Robô é obrigado a esperar as fontes do Google carregarem antes de tirar a foto
        await page.evaluateHandle('document.fonts.ready');
        
        // A SOLUÇÃO DAS IMAGENS: Carregamento tolerante com limite seguro
        await page.evaluate(async () => {
            const images = Array.from(document.querySelectorAll('img'));
            await Promise.all(images.map(img => {
                if (img.complete && img.naturalHeight !== 0) return Promise.resolve();
                return new Promise((resolve) => {
                    const timer = setTimeout(resolve, 25000); // Se a IA demorar mais de 25s, ele pula a foto e não trava o PDF
