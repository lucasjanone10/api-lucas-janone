...
async function getCoverImageBase64(city) {
    ...
    try {
        ...
        // Se a Wikipedia falhar, puxa uma foto arquitetônica de banco de dados
        if(!imgUrl) {
            const seed = Math.floor(Math.random()*1000);
            imgUrl = `https://loremflickr.com/1200/800/${encodeURIComponent(cleanCity)},architecture,luxury/all?lock=${seed}`;
        }

        // Baixa a imagem e converte para texto para o PDF nunca perder a foto
...
app.post('/api/gerar-roteiro', async (req, res) => {
    try {
        ...
        console.log(`[LOG] Iniciando Pitch Deck V2 para: ${nomeCliente} - Destino: ${destino}`);
        ...
        // FAXINEIRO DE JSON: Evita travamentos se a IA enviar formatações indesejadas
        let jsonStr = result.response.text();
        jsonStr = jsonStr.replace(/```json/g, '').replace(/```/g, '').trim();
        const dados = JSON.parse(jsonStr);

        console.log(`[LOG] Textos blindados extraídos. Gerando Design Editorial...`);
        ...
        <head>
            ...
            <style>
                ...
                /* ESTRUTURA DO SLIDE E MOLDURA DE LUXO */
                .page { 
                ...
                /* Moldura fina tipo Passe-partout */
                .page::after {
                ...
                /* A NOVA TIMELINE ELEGANTE PARA OS DIAS */
