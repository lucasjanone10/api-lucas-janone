...
app.post('/api/gerar-roteiro', async (req, res) => {
    try {
        ...
        // O RADAR: Buscando dinamicamente qual IA está liberada na sua chave do Google
        const url = `https://generativelanguage.googleapis.com/v1beta/models?key=${process.env.GEMINI_API_KEY}`;
        const apiResp = await fetch(url);
        const apiData = await apiResp.json();
        
        let targetModel = "gemini-1.5-flash"; // Fallback caso dê erro na busca
        if (apiData.models) {
            // Filtra todas as IAs Gemini que sabem gerar textos
            ...
            if (availableModels.length > 0) {
                // Tenta pegar o modelo 'flash' (mais rápido para textos), ou pega o primeiro que funcionar
        ...
        // Passando o modelo detectado automaticamente para a API
