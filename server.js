...
// CORS ULTRA-LIBERADO (Sem frescuras)
app.use(cors());
app.use(express.json());

// Rota de Saúde para a Render não achar que o servidor morreu
app.get('/', (req, res) => res.send('API Lucas Janone - Status: Operante 🚀'));
...
async function getCoverImageBase64(primeiraCidade) {
    ...
    try {
        ...
        console.warn(`[AVISO] Imagem falhou. Usando fallback.`);
        return null;
    }
}
...
app.post('/api/gerar-roteiro', async (req, res) => {
    try {
        ...
        if (numDias > 20) numDias = 20; // Trava máxima absoluta para a RAM não explodir

        console.log(`[LOG] Roteiro Ultra-Light: ${nomeCliente} | Destinos: ${destinoFormatado} | Dias: ${numDias}`);
        ...
        2. Seja EXTREMAMENTE BREVE nos textos para economizar memória.
        ...
        // EXTRATOR BLINDADO DE JSON (Garante que se a IA enviar lixo, a gente ignora)
        ...
        // DIETA EXTREMA DO PUPPETEER: Comando "--single-process" impede o consumo excessivo de RAM
        ...
        console.log(`[LOG] SUCESSO ANTI-DESMAIO! PDF gerado: ${pdfUrl}`);
        res.json({ pdfUrl });
...
app.listen(PORT, () => {
    console.log(`✅ Robô V4 (Anti-Desmaio) rodando na porta ${PORT}`);
});