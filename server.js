...
app.post('/api/gerar-roteiro', async (req, res) => {
    try {
        ...
        const prompt = `Você é Lucas Janone, um renomado especialista em viagens de luxo acessível. Crie um roteiro premium incrivelmente persuasivo e bem estruturado.
        ...
        - LINKS CLICÁVEIS: Sempre que mencionar um hotel, restaurante ou atração, coloque um link real (tag <a>) para o TripAdvisor ou site oficial.
        - CITAÇÃO: Logo após a introdução, adicione uma frase inspiradora sobre viagens ou sobre ${destino}. Use: <blockquote class="quote">"Frase" <br><strong>- Autor</strong></blockquote>
        ...
        4. Roteiro Dia a Dia (Manhã, Tarde, Noite). Insira a frase: "Como eu sempre ensino aos meus alunos da Mentoria, o segredo de viajar bem é..."
        ...
        // INJEÇÃO DE IMAGENS - PROMPT OTIMIZADO PARA A IA GERADORA DE FOTOS
        let imageCounter = 1;
        roteiroHTML = roteiroHTML.replace(/<h2(.*?)>(.*?)<\/h2>/g, (match) => {
            const query = encodeURIComponent(`beautiful luxury travel photography scenery landmark tourist spot in ${destino} day ${imageCounter}`);
            const imgUrl = `https://image.pollinations.ai/prompt/${query}?width=800&height=400&nologo=true`;
            imageCounter++;
            return `${match}\n<div class="img-container"><img class="day-image" src="${imgUrl}" alt="Visual incrível de ${destino}"></div>`;
        });

        const coverQuery = encodeURIComponent(`beautiful cinematic luxury travel photography ${destino} famous landmark`);
        const coverImageUrl = `https://image.pollinations.ai/prompt/${coverQuery}?width=1200&height=1600&nologo=true`;

        // O NOVO DESIGN PREMIUM (Fontes Elegantes, Texturas e Espaçamentos de Revista)
        const htmlTemplate = `
        <!DOCTYPE html>
        <html lang="pt-BR">
        <head>
            <meta charset="UTF-8">
            <style>
                /* Importando a fonte limpa (Inter) e a fonte de luxo (Playfair Display) */
                ...
                /* TÍTULOS PREMIUM (Estilo Editorial) */
                h2 { 
                    font-family: 'Playfair Display', serif; 
                    color: #0f172a; font-size: 32px; 
                    text-align: center;
                    margin-top: 0; margin-bottom: 30px;
                    page-break-before: always; 
                    padding-top: 50px; 
                }
                /* A linhazinha elegante debaixo do título */
                ...
                /* Subtítulos mais sofisticados */
                h3 { 
                    font-family: 'Inter', sans-serif; color: #FF6B35; font-size: 18px; 
                ...
                /* IMAGENS CORRIGIDAS COM FUNDO DE SEGURANÇA */
        ...
        const browser = await puppeteer.launch({ 
            headless: 'new',
            args: ['--no-sandbox', '--disable-setuid-sandbox', '--disable-web-security'] // Liberando segurança rigorosa para carregar imagens externas sem erro
        });
        ...
        // NOVO FREIO DE MÃO ABSOLUTO: O robô vai dar um tempo extra de segurança obrigatório para as fotos brotarem na tela
        ...
        await page.evaluate(async () => {
            ...
            await Promise.all(images.map(img => {
                ...
                return new Promise((resolve) => {
                    ...
                    img.addEventListener('error', resolve); // Se a imagem der erro, ele não trava, ele avança
                });
            }));
        });
