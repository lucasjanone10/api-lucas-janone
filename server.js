...
app.post('/api/gerar-roteiro', async (req, res) => {
    try {
        ...
        - PROIBIDO USAR CAPS LOCK. Nunca escreva frases ou títulos inteiros em maiúsculas.
        - Use <h3> para os turnos no formato exato: <h3>🌅 Manhã: Nome da Atividade</h3>, <h3>☀️ Tarde: Nome da Atividade</h3>, <h3>🌙 Noite: Nome da Atividade</h3>.
        ...
        // INJEÇÃO DE IMAGENS RÁPIDAS (Buscando fotos reais instantâneas em vez de gerar por IA para não travar)
        let imageCounter = 1;
        const destinoLimpo = encodeURIComponent(destino.trim().replace(/\s+/g, ''));
        roteiroHTML = roteiroHTML.replace(/<h2(.*?)>(.*?)<\/h2>/g, (match) => {
            // LoremFlickr é um banco de dados leve e rápido. Ele não vai derrubar o servidor.
            const imgUrl = `https://loremflickr.com/800/400/${destinoLimpo},landmark,travel?lock=${imageCounter}`;
            imageCounter++;
        ...
        // Renderização garantida sem timeout extremo
        await page.setContent(htmlTemplate, { waitUntil: 'networkidle2', timeout: 60000 });
        
        // Aguarda carregamento das imagens externas reais (muito mais rápido)
